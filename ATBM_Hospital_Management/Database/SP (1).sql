...
BEGIN
    ...
    SELECT 
        ...
        CHUYEN_KHOA
    FROM NHAN_VIEN
    WHERE UPPER(TRIM(MA_NV)) = UPPER(TRIM(v_user));
...
BEGIN
    ...
    OPEN p_cursor FOR
        ...
        FROM BENH_NHAN b
        JOIN NHAN_VIEN n ON UPPER(TRIM(b.MA_BN)) = UPPER(TRIM(n.MA_NV))
        WHERE UPPER(TRIM(b.MA_BN)) = UPPER(TRIM(v_user));
...
BEGIN
    ...
    WHERE UPPER(TRIM(MA_BN)) = UPPER(TRIM(v_user));
...
-- Thêm stored procedure bật toàn bộ audit policy của schema ADMIN_PH2
CREATE OR REPLACE PROCEDURE SP_ENABLE_ALL_AUDIT
AS
BEGIN
    FOR r IN (SELECT OBJECT_SCHEMA, OBJECT_NAME, POLICY_NAME FROM DBA_AUDIT_POLICIES WHERE OBJECT_SCHEMA = 'ADMIN_PH2') LOOP
...
-- Thêm stored procedure tắt toàn bộ audit policy của schema ADMIN_PH2
CREATE OR REPLACE PROCEDURE SP_DISABLE_ALL_AUDIT
AS
BEGIN
    FOR r IN (SELECT OBJECT_SCHEMA, OBJECT_NAME, POLICY_NAME FROM DBA_AUDIT_POLICIES WHERE OBJECT_SCHEMA = 'ADMIN_PH2') LOOP
