...
{
    public partial class GrantView : System.Windows.Forms.UserControl
    {
        ...
        private DropDownComboBox _cmbRoleToGrant;
        ...
        public GrantView()
        {
            ...
            cmbGrantMode.Items.AddRange(new object[] { "SYSTEM PRIVILEGE", "OBJECT PRIVILEGE", "ROLE" });
            cmbPrincipalType.SelectedIndex = 0;
            cmbGrantMode.SelectedIndex = 0;

            cmbPrincipalType.SelectedIndexChanged += cmbPrincipalType_SelectedIndexChanged;
            cmbGrantMode.SelectedIndexChanged += cmbGrantMode_SelectedIndexChanged;
        ...
        private void ClearDynamicRow()
        {
            ...
            _cmbObjectName = null; _cmbPrivType = null; _lstColumns = null; _cmbRoleToGrant = null;
            tlpMain.RowStyles[3] = new RowStyle(SizeType.Absolute, 52F);
            tlpMain.Height = 208;
            tlpMain.PerformLayout();
        }

        private void cmbGrantMode_SelectedIndexChanged(object sender, EventArgs e)
        {
            _currentMode = cmbGrantMode.SelectedItem?.ToString() ?? "SYSTEM PRIVILEGE";
            ClearDynamicRow();
            switch (_currentMode)
            {
        ...
        private void ShowObjectPrivPanel()
        {
            ...
            tlpMain.Height = 208 + 200; // 3 fixed rows + expandable object priv area
            tlpMain.PerformLayout();
        ...
        private void ShowRolePanel()
        {
            ...
            _cmbRoleToGrant = new DropDownComboBox { Anchor = AnchorStyles.Left | AnchorStyles.Right, DropDownStyle = ComboBoxStyle.DropDownList, Font = new System.Drawing.Font("Segoe UI", 9.5F), MaxDropDownItems = 10 };
            try { DataTable dt = _dbaService.GetRolesDetailed(); foreach (DataRow row in dt.Rows) _cmbRoleToGrant.Items.Add(row["ROLE"].ToString()); if (_cmbRoleToGrant.Items.Count > 0) _cmbRoleToGrant.SelectedIndex = 0; }
            catch (Exception ex) { MessageBox.Show("Error loading roles: " + ex.Message); }
            tlpMain.RowStyles[3] = new RowStyle(SizeType.Absolute, 52F);
            tlpMain.PerformLayout();
            tlpMain.Controls.Add(_lblDynamic, 0, 3);
            tlpMain.Controls.Add(_cmbRoleToGrant, 1, 3);
        }
